...
| product_name | `DaVinci Resolve` (must match `About.vue` `name`) |
...
bash skills/logo-finder/scripts/commons.sh --url 'https://upload.wikimedia.org/.../file.svg' work/web/public/brands/file.svg

# Search manifest only
bash skills/logo-finder/scripts/commons.sh --search davinci

# About.vue: logo: null -> SVGL, then commons
...
2. Prefer **SVG** over PNG/JPG. If only PNG exists (common for Apple app icons), save as `{slug}.png` and wire that path in `About.vue`.
...
4. Add row to `commons-logos.tsv`, run `commons.sh`, wire `logo: '/brands/{slug}.svg'` in `About.vue`.
