...
Per-frame sampling is required — a single tween will not react. Pre-extract bands with `extract-audio-data.py` from the hyperframes upstream repo (`skills/gsap/scripts/extract-audio-data.py`):
